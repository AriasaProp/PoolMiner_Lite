...
const uint32_t *hex_array::data() const {
	return arr.data();
}
...
void hashing::innerHash() {
  Sha256::Initialise (&cntx);
  memset (B + 80, 0, 3);

  for (i = 0; i < 4; i++) {
    B[83] = i + 1;
    Sha256::Update (&cntx, B, 84);
    Sha256::Finalise (&cntx, (uint8_t*)(X + (i * 8)));
  }
...
*/
  ...
  memset (B+128, 0, 3);
  B[131] = 1;
  Sha256::Update (&cntx, B, 132);
  Sha256::Finalise (&cntx, H);
}
